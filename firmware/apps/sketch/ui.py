...
def draw_background():
    ...
    screen.brush = color.rgb(170, 45, 40)
    screen.draw(shape.rectangle(0, 0, 160, 120))

    # draw the embossed gold logo
    screen.font = font
    w, _ = screen.measure_text("MonaSketch")
    screen.brush = color.rgb(240, 210, 160)
    screen.text("MonaSketch", 80 - (w / 2) - 1, -1)
    screen.brush = color.rgb(190, 140, 80, 100)
    screen.text("MonaSketch", 80 - (w / 2), 0)

    # draw the canvas area grey background and screen shadows
    screen.brush = color.rgb(210, 210, 210)
    screen.draw(shape.rounded_rectangle(*canvas_area, 6))
    screen.brush = color.rgb(180, 180, 180)
    screen.draw(
    ...
    screen.brush = color.rgb(255, 255, 255, 100)
    screen.draw(
        shape.rectangle(
...
def draw_dial(angle, pos):
    ...
    screen.brush = color.rgb(0, 0, 0, 40)
    screen.draw(shape.circle(pos[0] + offset * 1.5, pos[1], radius + 2))

    # draw the dial shaft
    screen.brush = color.rgb(150, 160, 170)
    screen.draw(shape.circle(pos[0] + offset, pos[1], radius))

    # draw the dial surface
    screen.brush = color.rgb(220, 220, 230)
    screen.draw(shape.circle(*pos, radius))

    # draw the animated ticks around the dial edge
    screen.brush = color.rgb(190, 190, 220)
    ticks = 20
    for i in range(ticks):
        ...
        screen.draw(shape.line(*inner, *outer, 1.5))
...
def draw_cursor(cursor):
    ...
    screen.brush = brush.xor(i, i, i)
    screen.draw(shape.rectangle(cx + 2, cy, 2, 1))
    screen.draw(shape.rectangle(cx - 3, cy, 2, 1))
    screen.draw(shape.rectangle(cx, cy + 2, 1, 2))
    screen.draw(shape.rectangle(cx, cy - 3, 1, 2))
