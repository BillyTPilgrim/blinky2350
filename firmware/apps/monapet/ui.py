...
def background(mona):
    ...
    screen.brush = color.rgb(30, 50, 70)
    screen.draw(shape.rectangle(0, 0, 160, floor_y))

    # animate the wallpaper
    screen.brush = color.rgb(30, 40, 20)
    mx = (mona_x - 80) / 2
    for y in range(8):
        for x in range(19):
            if (x + y) % 2 == 0:
                ...
                screen.draw(shape.rectangle(
                    x * 10 - mx, y * 10 - 3, xo + 4, yo + 4))

    # draw the picture frame
    px = 140 - mx
    screen.brush = color.rgb(80, 90, 100, 100)
    screen.draw(shape.line(px + 2, 20 + 2, px + 20, 15, 1))
    screen.draw(shape.line(px + 35 + 2, 20 + 2, px + 20, 15, 1))
    screen.brush = color.rgb(30, 40, 50, 100)
    screen.draw(shape.rectangle(px + 1, 20 + 1, 38, 28))
    screen.brush = color.rgb(50, 40, 30, 255)
    screen.draw(shape.rectangle(px, 20, 38, 28))
    screen.brush = color.rgb(120, 130, 140, 255)
    screen.draw(shape.rectangle(px + 2, 20 + 2, 38 - 4, 28 - 4))
    portrait = mona._animations["heart"].frame(7)  # noqa: SLF001
    screen.blit(portrait, px + 8, 20)

    # draw the skirting board
    screen.brush = color.rgb(80, 90, 100, 150)
    screen.draw(shape.rectangle(0, floor_y - 5, 160, 5))
    screen.draw(shape.rectangle(0, floor_y - 4, 160, 1))
    ...
    floor.brush = color.rgb(30, 40, 20)
    floor.draw(shape.rectangle(0, 0, 160, 120 - floor_y))

    # draw angled "floorboard" lines centered on mona
    floor.brush = color.rgb(100, 200, 100, 25)
...
def draw_header():
    ...
    screen.draw(shape.rounded_rectangle(40, -5, 160 - 80, 18, 3))
...
def draw_button(x, y, label, active):
    ...
    screen.brush = color.rgb(255, 255, 255, 255 if active else 150)
...
def draw_bar(name, x, y, amount):
    ...
    screen.brush = outline_brush
    screen.draw(shape.rounded_rectangle(x, y, bar_width, 12, 3))

    # draw the bar background
    screen.brush = outline_brush
    screen.draw(shape.rounded_rectangle(x + 14, y + 3, bar_width - 17, 6, 2))
    ...
    screen.draw(shape.rounded_rectangle(x + 14, y + 3, fill_width, 6, 2))

    screen.brush = color.rgb(210, 230, 250, 50)
    screen.draw(shape.rounded_rectangle(x + 15, y + 3, fill_width - 2, 1, 1))
...
def shadow_text(text, y, sx=0, ex=160):
    temp = screen.brush
    screen.brush = color.rgb(0, 0, 0, 100)
