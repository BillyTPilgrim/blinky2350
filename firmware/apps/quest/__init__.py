...
def update():
  ...
  # clear the screen
  screen.brush = color.rgb(35, 41, 37)
  ...
  if _last_task_completed_at:
    ...
    if io.ticks - _last_task_completed_at < zoom_speed:
      ...
      # draw message bubble
      screen.brush = color.rgb(46, 160, 67, 200)
      lw_corners = (4, 4, 0, 0) if lw < mw else (4, 4, 4, 4)
      mw_corners = (4, 4, 4, 4) if lw < mw else (0, 0, 4, 4)
      screen.draw(shape.rounded_rectangle(80 - (lw / 2) - 4, 2, lw + 8, 18, *lw_corners))
      screen.draw(shape.rounded_rectangle(80 - (mw / 2) - 4, 20 , mw + 8, 12, *mw_corners))

      # draw task label and message
      screen.brush = color.rgb(255, 255, 255, 255)
      screen.font = large_font
